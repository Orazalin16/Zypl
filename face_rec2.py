...
try:
    data = pickle.loads(open(os.getcwd() + '\encodingss.pickle', 'rb').read())    #encodings here
except FileNotFoundError:
    knownEncodings, knownNames = [], []
    imagePaths = list(paths.list_images(os.getcwd() + '/original/'))    #dataset here
    for (i, imagePath) in enumerate(imagePaths):
        print("one cycle\n")
        print('{}/{}'.format(i+1, len(imagePaths)), end=', ')
        image, name = cv2.imread(imagePath), imagePath.split(os.path.sep)[8][0:9]
...
stream = cv2.VideoCapture('Test_2.mp4')    #test video here
writer = None
...
while True:
    ...
    for encoding in encodings:
        ...
        if True in votes:
            names.append(str(Counter([name for name, vote in list(zip(data['names'], votes)) if vote == True]).most_common()[0][0]))
    ...
    if writer is None:
        writer = cv2.VideoWriter(os.getcwd() + 'Test.mp4', cv2.VideoWriter_fourcc(*'MJPG'), 24, (frame.shape[1], frame.shape[0]), True)
    writer.write(frame)
